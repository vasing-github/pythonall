...
try:
    with connection.cursor() as cursor:
        # 打开 Excel 文件
        workbook = load_workbook(filename='zhongdian.xlsx')
        ...
        for row in sheet.iter_rows(min_row=2, values_only=True):  # 假设第一行是列名
            ...
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, '2', %s)
            """
            print(sql)
            cursor.execute(sql, (row[1], row[2], contact_name, contact_phone, row[4], row[8], row[5], row[6], row[7], row[9], row[10], row[12], row[11],  row[14]))
