...
if __name__ == '__main__':
    ...
    # sendMsg(f'## 机构职能页监测：\n\n',f'今日检测到<font color=\"warning\">{x}</font>个单位链接有误')
    # if  not_open_list :
    #     sendMsgErrorList(f'## 机构职能页监测：\n\n以下位置未发布',not_open_list)
    # if  error_state_list:
    #     sendMsgErrorList(f'## 机构职能页监测：\n\n以下位置链接有误，请检查',error_state_list)
    # if  zzgk_error_list:
    #     sendMsgErrorList(f'## 机构职能页监测：\n\n以下单位机关简介中无领导分管政务公开，请检查', zzgk_error_list)


    # if not not_open_list and not error_state_list and not zzgk_error_list:
    #     print('---')
    #     sendMsg(f'## 机构职能页监测：\n\n', f'今日检测到<font color=\"warning\">0</font>个单位链接有误')


    print(not_open_list)
    print(error_state_list)
    print(zzgk_error_list)
